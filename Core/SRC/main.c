...
int main(void)
{
  ...
  MX_TIM4_Init();
  /* USER CODE BEGIN 2 */
  uint32_t last_dbg = 0;   // throttle for the debug log

  bootloader_init();        //底层：串口接收（中断双缓冲）+ flash 擦写，只管收
  App_bootloader_init();    //上层：升级状态机（等待 start:len 命令）
  ...
  while (1)
  {
    ...
    {
      Key_Event_Flag = 0;
      App_bootloader_force_finish();
    }

    App_bootloader_work();   //升级状态机：解析 start:len → 收帧写 flash → 校验 → 跳转

    if (HAL_GetTick() - last_dbg >= 3000)   // debug log every 3s
    {
