...
| 版本 | 位置 | 说明 | 实测参数（9600 baud） |
|---|---|---|---|
| **中断双缓冲版** | **仓库根目录**（`./`） | RX 中断 + IDLE 空闲检测，双缓冲 2×521B | 单次 ≤256 字节，块间延时 ≥100ms |
| **DMA 版** | **`dma-version/`** 子目录 | 使用 DMA 双缓冲接收 | 单次 ≤256 字节，块间延时 ≥1ms |
...
**串口烧录约束**：两个版本在 9600 baud 下实测稳定上限都是单次 ≤256 字节，但 DMA 版块间延时只需 ≥1ms（中断版需要 ≥100ms），传输速度更快。
