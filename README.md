...
**原因**：bootloader 用  双缓冲接收，单块 521B，若上位机连续高速发送，接收中断可能处理不过来触发 ORE（Overrun Error），虽然 Int_bootloader.c:86-89 有 ORE 自愈（清标志 + 重新挂接收），但连续丢字节会导致校验求和不匹配。建议每次发 ≤256 字节后等 100ms 再发下一块。
