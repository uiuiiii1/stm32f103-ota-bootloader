...
上位机发送固件时，为避免 USART1 RX 中断双缓冲来不及处理导致丢字节（ORE），必须遵守以下速率约束：

| 参数 | 上限 |
|---|---|
| 单次发送块大小 | ≤ 256 字节 |
| 块间延时 | ≥ 100 ms |
| 波特率 | 9600（可调高，但块间延时需相应缩短） |

**原因**：bootloader 用 `HAL_UARTEx_ReceiveToIdle_IT` 双缓冲接收，单块 521B，若上位机连续高速发送，接收中断可能处理不过来触发 ORE（Overrun Error），虽然 Int_bootloader.c:86-89 有 ORE 自愈（清标志 + 重新挂接收），但连续丢字节会导致校验求和不匹配。建议每次发 ≤256 字节后等 100ms 再发下一块。
