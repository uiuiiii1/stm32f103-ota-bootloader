...
//flash 擦除/写入全部放在主循环（bootloader_process_frame），ISR 只做微秒级的搬数据，
...
void bootloader_init(void)
{
    ...
    HAL_UARTEx_ReceiveToIdle_IT(&huart1, rx_buf[rx_active], BOOTLOADER_USART_REC_BUFF_LEN);
}

//空闲帧接收回调
//注意名字必须是 HAL_UARTEx_RxEventCallback（带 Ex）！HAL_UARTEx_ReceiveToIdle_IT 在
//IDLE 事件时调用的是带 Ex 的这个，见 stm32f1xx_hal_uart.c:2713 的 weak 默认实现。
//之前写成 HAL_UART_RxEventCallback（无 Ex）是个独立函数，HAL 永远不会调它，导致收到数据也没回应。
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart,uint16_t Size)
{
    ...
    //把本帧搬进暂存区，置标志；主循环 bootloader_process_frame() 负责写 flash
...
void bootloader_jump_to_app(void)
{
    ...
    //关中断
    __disable_irq();
    /* 顺序敏感：HAL_RCC_DeInit() 内部会调用 HAL_InitTick()，而本工程 HAL 时基
     * 用的是 TIM4（见 stm32f1xx_hal_timebase_tim.c），该函数会重启 TIM4 并重新
     * 使能 TIM4_IRQn。因此下面的 NVIC 清理必须放在 HAL_RCC_DeInit() 之后执行；
     * 一旦调换顺序，TIM4 会被重新使能，跳进 app 后 __enable_irq() 会把积压的
     * TIM4 中断派发给 app 向量表里的弱符号 Default_Handler（B . 死循环），
     * app 当场卡死、完全无反应。 */
